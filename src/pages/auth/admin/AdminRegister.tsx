...
const AdminRegister = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // ยิงไปที่ path /create-admin ตามที่ backend กำหนด
      await axios.post(`${import.meta.env.VITE_BASE_URL}/create-admin`, {
      ...
      await Swal.fire({
        icon: "success",
        title: "ลงทะเบียนผู้ดูแลระบบสำเร็จ",
    ...
    <div className="flex justify-center items-center min-h-[80vh] font-roboto py-10">
      <div className="card w-96 bg-white/80 backdrop-blur-md shadow-2xl border border-white/50">
        <div className="card-body">
          <h2 className="card-title text-3xl font-bold font-pacifico text-rose-600 text-center w-full mb-4">Admin Register</h2>
          <form onSubmit={handleSubmit} className="space-y-3">
            <div className="form-control">
              <label className="label">
                <span className="label-text font-semibold text-slate-600">ชื่อผู้ใช้ (Admin)</span>
              </label>
              <input
                type="text"
                name="username"
                className="input input-bordered bg-white/50 focus:bg-white focus:border-rose-400 transition-all"
            ...
            <button type="submit" className="btn bg-rose-500 hover:bg-rose-600 border-none text-white w-full mt-6 rounded-full shadow-lg shadow-rose-200" disabled={loading}>
              {loading ? "กำลังลงทะเบียน..." : "ลงทะเบียน Admin"}
            </button>
          </form>
          <p className="text-center mt-6 text-sm text-slate-500">
            มีบัญชีอยู่แล้ว? <Link to="/login" className="link text-rose-600 font-bold hover:text-rose-700 no-underline">เข้าสู่ระบบ</Link>
          </p>
        </div>
      </div>
    </div>
